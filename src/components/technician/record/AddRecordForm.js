...
class Addrecordform extends Component {
  ...
  validateStateInput = () => {
    ...
    if (
      !fields["meter_reading_month_year"] ||
      fields["meter_reading_month_year"] === null
    ) {
    ...
    if (
      !fields["meter_reading_date"] ||
      fields["meter_reading_date"] === null
    ) {
  ...
  checkLocation = () => {
    ...
    if (
      Math.abs(m_lat) - 0.000015 <= Math.abs(t_lat) &&
      Math.abs(t_lat) <= Math.abs(m_lat) + 0.000015
    ) {
      lat_b = true;
    }
    if (
      Math.abs(m_long) - 0.000015 <= Math.abs(t_long) &&
      Math.abs(t_long) <= Math.abs(m_long) + 0.000015
    ) {
      long_b = true;
    }
    return lat_b && long_b;
        ...
        axios
          .post(
            api,
            {
              ...
              meter_reading_month_year: moment(
                this.state.fields["meter_reading_month_year"]
              ).format("YYYY-MM-01"),
              meter_reading_date: moment(
                this.state.fields["meter_reading_date"]
              ).format("YYYY-MM-DD"),
            },
            {
              headers: {
          ...
          <div className="container-fluid">
            <div className="card card-outline card-primary">
              <div className="card-header">
                <h3 className="card-title">Add Meter Record</h3>
              </div>
              {this.state.isLoaded ? (
